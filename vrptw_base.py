...
class VrptwGraph:
    def __init__(self, file_path, mode=1, rho=0.1):
        super()
        self.mode = mode
        if mode == 1 or mode == 2:
            ...
            self.node_num, self.nodes, self.node_dist_mat, self.vehicle_num, self.vehicle_capacity, self.node_path_mat \
                = self.mode_3_create(file_path)
        elif mode == 4:
            self.node_num, self.nodes, self.node_dist_mat, self.vehicle_num, self.vehicle_capacity, self.node_path_mat \
                = self.mode_4_create(file_path)
    ...
    def mode_3_create(self, file_path):
        node_list = []
        ...
        for i in range(node_num):
            node_path_mat.append([])
        for i in range(node_num):
            for j in range(node_num):
                node_path_mat[i].append([])
        ...
        for i in range(node_num):
            ...
            for j in range(i + 1, node_num):
                node_b = nodes[j]
                node_dist_mat[i][j], node_path_mat[i][j] = VrptwGraph.astar_find_dist(file_path, node_a, node_b)
                node_dist_mat[j][i] = node_dist_mat[i][j]
                node_path_mat[j][i] = node_path_mat[i][j]

        return node_num, nodes, node_dist_mat, vehicle_num, vehicle_capacity, node_path_mat

    def mode_4_create(self, file_path):
        dest_in_neighborhood = [9, 8, 10, 11, 12, 13, 14, 15]
        ...
        for i in range(node_num):
            ...
            for j in range(i + 1, node_num):
                node_b = nodes[j]
                if i in dest_in_neighborhood or j in dest_in_neighborhood:
                    node_dist_mat[i][j], node_path_mat[i][j] = VrptwGraph.in_neighbor_find_dist(file_path, node_a, node_b)
                else:
                    node_dist_mat[i][j], node_path_mat[i][j] = VrptwGraph.neighbor_find_dist(file_path, node_a, node_b)
                node_dist_mat[j][i] = node_dist_mat[i][j]
                node_path_mat[j][i] = node_path_mat[i][j]
    ...
    def create_from_file(self, file_path):
        ...
        for i in range(node_num):
            ...
            for j in range(i + 1, node_num):
                node_b = nodes[j]
                if self.mode == 1:
                    node_dist_mat[i][j] = VrptwGraph.euclidean_dist(node_a, node_b)
                elif self.mode == 2:
                    node_dist_mat[i][j] = VrptwGraph.manhattan_dist(node_a, node_b)
                node_dist_mat[j][i] = node_dist_mat[i][j]
    ...
    @staticmethod
    def neighbor_find_dist(file, node_a, node_b):
        return astar_neighbor.Astar(file, 50, 50, node_a, node_b)

    @staticmethod
    def in_neighbor_find_dist(file, node_a, node_b):
        return astar_neighbor.N_Astar(file, 50, 50, node_a, node_b)
    ...
    @staticmethod
    def manhattan_dist(node_a, node_b):
    ...
    def _cal_nearest_next_index(self, index_to_visit, current_index, current_load, current_time):
        ...
        for next_index in index_to_visit:
            ...
            if current_time + dist + wait_time + service_time + self.node_dist_mat[next_index][0] > self.nodes[
                0].due_time:
                continue
