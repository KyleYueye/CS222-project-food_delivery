...
def loadMap(file="./solomon-100/c101-neighborhood.txt"):
...
def getNeighbor(file, pos):
    ...
    # map = loadMap(file)
    # the point above
    if pos[0] >= 1 and map[pos[0]-1, pos[1]] != 1 and map[pos[0]-1, pos[1]] != 4 and map[pos[0]-1, pos[1]] != 6:
        neighbor.append((pos[0]-1, pos[1]))
    # the point on the right
    if pos[1] <= len(map[0])-2 and map[pos[0], pos[1]+1] != 1 and map[pos[0]-1, pos[1]] != 4 and map[pos[0]-1, pos[1]] != 6:
        neighbor.append((pos[0], pos[1]+1))
    # the point below
    if pos[0] <= len(map)-2 and map[pos[0]+1, pos[1]] != 1 and map[pos[0]-1, pos[1]] != 4 and map[pos[0]-1, pos[1]] != 6:
        neighbor.append((pos[0]+1, pos[1]))
    # the point on the left
    if pos[1] >= 1 and map[pos[0], pos[1]-1] != 1 and map[pos[0]-1, pos[1]] != 4 and map[pos[0]-1, pos[1]] != 6:
        neighbor.append((pos[0], pos[1]-1))
    return neighbor


def N_getNeighbor(file, pos):
    neighbor = []
    # map = loadMap(file)
    # the point above
...
def N_Astar(mapfile, rowNum, colNum, node_a, node_b):
    start = (int(node_a.x), int(node_a.y))
    goal = (int(node_b.x), int(node_b.y))
    frontier = q.PriorityQueue()
    frontier.put(start, 0)
    ...
    while not frontier.empty():
        ...
        if current == goal:
            break
        for next in N_getNeighbor(mapfile, current):
            new_cost = cost_so_far[current] + 1
            if next not in cost_so_far or new_cost < cost_so_far[next]:
                cost_so_far[next] = new_cost
                priority = new_cost + heuristic(goal, next)
